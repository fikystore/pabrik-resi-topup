...
app.get('/generate-qris', async (req, res) => {
    try {
        const nominal = parseInt(req.query.nominal) || 1000;

        // ── QRIS generator (TIDAK DISENTUH) ──
        function crc16(data) {
            let crc = 0xFFFF;
        ...
        function generateQrisString(nom) {
            ...
            const nomStr = parseInt(nom).toString();
            const amountField = "54" + nomStr.length.toString().padStart(2, "0") + nomStr;
            qris = qris.replace("5802ID", amountField + "5802ID");
            qris += "6304";
            return qris + crc16(qris);
        }
        // ── END QRIS (tidak disentuh) ──

        // Generate QR langsung jadi base64 di server, tanpa CDN
        const qrisStr = generateQrisString(nominal);
        const qrDataUrl = await QRCodeLib.toDataURL(qrisStr, {
            width: 440,
            margin: 2,
        ...
        const rupiah = 'Rp ' + parseInt(nominal).toLocaleString('id-ID');
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .header-sub { font-size: 16px; color: #555; margin-bottom: 20px; }
...
<div class="card">
  ...
  <div class="content">
    ...
    <div class="qr-wrapper">
      ...
      <img src="${qrDataUrl}" width="440" height="440" />
    </div>
    <div style="font-size:42px;font-weight:800;color:#1565C0;margin-top:25px;letter-spacing:1px;">
      ${rupiah}
    </div>
    <div class="tg-footer">
      <div class="tg-icon">
      ...
      <span class="tg-handle">@PusatTopup_bot</span>
        ...
        // Puppeteer hanya render HTML statis, tidak perlu tunggu network
